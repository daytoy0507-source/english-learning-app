  ...
  container.innerHTML = `
    ...
    <div class="main-content">
      ...
      <div class="page-content">
        ...
        ${getFilteredWords().length === 0 ? '<p class="empty-state">暂无单词，点击上方按钮添加</p>' : ''}
      </div>
      ` : ''}

      <!-- 背诵页面 -->
      ${app.currentPage === 'study' ? `
      <div class="page-content">
        ${!app.studying ? `
      ...
      <div class="page-content">
        ...
        <div class="words-list">
          ${app.errorWords.map(error => {
            const word = getWordById(error.word_id);
            return `
...
function getFilteredWords() {
  if (!app.searchWord) return app.words;
  return app.words.filter(w => 
    w.word.toLowerCase().includes(app.searchWord.toLowerCase())
  );
}

function getWordById(id) {
  return app.words.find(w => w.id === id);
}

function getProgressPercent() {
  if (app.studyWords.length === 0) return 0;
  return Math.round((app.currentIndex / app.studyWords.length) * 100);
}

function getEmbedUrl() {
  const videoId = extractVideoId(app.videoUrl);
...
async function startStudy(type) {
  ...
  if (type === 'all') {
    app.studyWords = [...app.words].sort(() => Math.random() - 0.5);
  } else if (type === 'review') {
    const today = new Date().toISOString().split('T')[0];
    app.studyWords = app.words.filter(w => !w.last_studied || w.last_studied !== today);
  } else if (type === 'errors') {
    const wordIds = app.errorWords.map(e => e.word_id);
    app.studyWords = app.words.filter(w => wordIds.includes(w.id));
  }
  
  app.studyWords = app.studyWords.slice(0, 20); // 每次最多20个
...
async function loadWords() {
  ...
  app.masteredWords = app.words.filter(w => w.level >= 4).length;
}
